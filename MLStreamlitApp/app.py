...
def detect_id_like_columns(df: pd.DataFrame) -> list[str]:
    """Find columns that look like record identifiers instead of useful features."""
    id_like = []
    for col in df.columns:
        col_lower = col.lower()
        unique_ratio = df[col].nunique(dropna=True) / max(len(df), 1)

        # Columns with "id" in the name or almost all unique values
        # are usually labels/identifiers, not predictive features.
...
def safe_read_csv(file) -> pd.DataFrame:
    """Read uploaded CSV and stop the app with a message if it fails."""
    try:
...
# Do the rest of the workflow only after a file is uploaded.
if uploaded_file is not None:
    df = safe_read_csv(uploaded_file)

    # Quick preview so the user can confirm the upload worked.
    st.subheader("Dataset Preview")
    st.dataframe(df.head())

    # Small summary stats at the top.
    c1, c2, c3 = st.columns(3)
    c1.metric("Rows", df.shape[0])
    c2.metric("Columns", df.shape[1])
    c3.metric("Missing Values", int(df.isna().sum().sum()))

    # Let the user choose which column to predict.
    ...
    if target:
        ...
        # Show class balance for the target variable.
        ...
        with chart_right:
            ...
            # Basic check to see if the chosen target makes sense for classification.
            if target_unique < 2:
                ...
                st.success("This target looks appropriate for a classification task.")
        ...
        # Separate numeric and categorical columns so they can be cleaned differently.
        ...
        # Fill missing categorical values with a label so they are not lost.
        for col in categorical_cols:
            X[col] = X[col].fillna("Missing")

        # Convert text categories into numeric columns for modeling.
        X = pd.get_dummies(X, drop_first=False)

        # If the target is text, encode it into numeric labels.
        label_encoder = None
        ...
        # Show different controls depending on the selected model.
        if model_name == "Logistic Regression":
            ...
            model_description = (
                ...
                "It works best when classes can be separated by fairly simple patterns."
            )

            hyperparameter_notes = (
                f"- **C = {c_value:.2f}**: Controls regularization strength. "
                "Lower values apply stronger regularization.\n"
            ...
            model_description = (
                ...
                "They are useful for capturing less linear patterns."
            )

            hyperparameter_notes = (
                f"- **Max Depth = {max_depth}**: Limits how deep the tree can grow. "
                "Smaller values may reduce overfitting.\n"
                f"- **Min Samples Split = {min_samples_split}**: Minimum number of samples required "
                "before a node can be split."
            )
        ...
        if train_button:
            try:
                ...
                with left_results:
                    ...
                    # Plot confusion matrix to show which classes are mixed up.
                    ...
                    # ROC curve only makes sense here for binary classification.
                ...
                with right_results:
                    ...
                    if acc >= 0.80:
                        st.success(
                            "The model is performing strongly on this dataset. "
                            "The selected features seem useful for predicting the target."
                        )
                    elif acc >= 0.60:
                        st.info(
                            "The model shows moderate performance. It captures some useful patterns, "
                            "but results could likely improve with different settings or features."
                        )
                    else:
                        st.warning(
                            "The model shows relatively low performance on this dataset. "
                            "This may mean the selected features are not strongly related to the target, "
                            "or that a different model or hyperparameter setting may work better."
                        )

                    st.markdown("**Confusion matrix interpretation:**")
                    st.write(
                        "The confusion matrix shows where the model makes correct predictions "
                        "and where it confuses one class with another. A stronger model would "
                        "show more values concentrated on the diagonal."
                    )
